...
def main(): 
    ...
    transforms_ = [ transforms.Resize((opt.img_height, opt.img_width), Image.BICUBIC),
                    transforms.ToTensor(),
                    transforms.Normalize((0.5,0.5,0.5), (.5,0.5,0.5))
  ]
    dataloader = DataLoader(ImageDataset("data/train/",  "data/val/", transforms_=transforms_),
                            batch_size=opt.batch_size, shuffle=True, num_workers=opt.n_cpu)
    val_dataloader = DataLoader(ImageDataset("data/train/", "data/val/", transforms_=transforms_, mode='val'),
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            prev_time = time.time()
            # Print log
            sys.stdout.write("\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, pixel: %f, adv: %f] ETA: %s lr: %f" %
                                                            (epoch, opt.n_epochs,
                                                            i, len(dataloader),
