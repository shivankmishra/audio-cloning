...
def wav2mel(filename):
    x, sr = librosa.load(filename)
    S = librosa.feature.melspectrogram(y=x, sr=sr, n_fft=N_FFT, hop_length=N_FFT // 4)
    S = np.log1p(S)
    return S, sr
...
def gram_over_time_axis(A, gram_mode):
    """
    Argument:
    A -- matrix of shape (1, n_C, n_H, n_W)

    Returns:
    GA -- Gram matrix of A along time axis, of shape (n_C, n_C) if gram_mode = 0
          Otherwise, gram matrix has shape (n_C * n_H, n_C * n_H)
    """
...
def compute_layer_style_loss(a_S, a_G, gram_mode):
    """
    Arguments:
    a_S -- tensor of dimension (1, n_C, n_H, n_W)
    a_G -- tensor of dimension (1, n_C, n_H, n_W)
    gram_mode -- type of gram matrix product to calculate
                 0 for Gatys et al., 1 for new axis
    ...
    GS = gram_over_time_axis(a_S, gram_mode)
    GG = gram_over_time_axis(a_G, gram_mode)
